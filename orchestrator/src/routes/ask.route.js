...
router.post("/generate-commit", async (req, res) => {
  try {
    ...
    // 2. Get the RAW Git Diff
    const { default: simpleGit } = await import("simple-git");
    const git = simpleGit(workspacePath);
    
    // 🟢 THE FIX: Add everything to staging so new/untracked files are actually sent to Gemini
    await git.add(".");
    const diff = await git.diff(["--staged"]);

    if (!diff || diff.trim().length === 0) {
        return res.json({ success: true, message: "Update files" });
    }

    // 3. 🧠 Ask Gemini to generate the message instantly!
    const prompt = `Write a single, concise git commit message (max 1 sentence) for these changes. DO NOT use markdown, quotes, or conversational text:\n\n${diff.substring(0, 5000)}`;
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] })
    ...
    const data = await response.json();
    let generatedMessage = data.candidates[0].content.parts[0].text.trim().replace(/^"|"$/g, '');
...
router.post("/list-branches", async (req, res) => {
  try {
   ...
   // Read the branches using simple-git
    ...
    // 1. Silently fetch from remote so we know about brand new branches!
    try {
       await git.fetch();
    } catch (e) {
       console.warn("Silent fetch failed:", e.message);
    }

    // 2. 🟢 THE FIX: Use ['-a'] to get ALL local and remote branches
    const branchSummary = await git.branch(['-a']);

    // 3. Clean up the names (strip "remotes/origin/" and remove duplicates)
    const cleanBranches = Array.from(new Set(
        branchSummary.all
          .map(b => b.replace(/^remotes\/origin\//, '').trim())
    )).filter(b => !b.startsWith('HEAD')); // Hide the 'HEAD -> main' pointer

    res.json({ 
      success: true, 
      all: cleanBranches,             // The cleaned, complete array
