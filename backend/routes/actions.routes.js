...
// 🟢 NEW: Keep the Database Active Repo perfectly synced with the Frontend UI
router.post("/set-active-repo", async (req, res, next) => {
  try {
    const { repoName } = req.body;
    if (!repoName) return res.json({ success: false });

    const workspace = await Workspace.findOne({ userId: req.userId });
    if (!workspace) return res.json({ success: false });

    // Fuzzy match the repo name to find the exact DB document
    const normalize = (str) => str ? str.toLowerCase().replace(/[-_\s]/g, "") : "";
    const allRepos = await Repo.find({ workspaceId: workspace._id });
    const targetRepo = allRepos.find(r => normalize(r.localPath).includes(normalize(repoName)));

    if (targetRepo) {
      await Workspace.findByIdAndUpdate(workspace._id, { activeRepoId: targetRepo._id });
    }
